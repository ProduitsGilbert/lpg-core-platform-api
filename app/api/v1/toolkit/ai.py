...
from app.domain.toolkit.ai_service import OPENROUTER_MODEL_ORDER, AIService
...
def _map_grok_http_error(exc: httpx.HTTPStatusError, action: str) -> HTTPException:
    status_code = exc.response.status_code if exc.response else status.HTTP_502_BAD_GATEWAY
    detail = ""
    if exc.response:
        try:
            ...
            detail = "<streaming response not consumed>"
    logger.error(
        "Grok request returned HTTP error",
        extra={"action": action, "status_code": status_code, "detail": detail[:500]},
    )
    return HTTPException(
        status_code=status.HTTP_502_BAD_GATEWAY,
        detail={
            "error": {
                "code": "GROK_ERROR",
                "message": f"Grok upstream call failed while attempting to {action}",
                "upstream_status": status_code,
            }
        },
    )
...
def _map_grok_request_error(exc: httpx.RequestError, action: str) -> HTTPException:
    logger.error(
        "Grok request failed",
        extra={"action": action, "error": str(exc)},
    )
...
@router.post(
    ...
    response_model=GrokImageGenerationResponse,
    responses={
        status.HTTP_200_OK: {"description": "Grok image generation completed successfully"},
        status.HTTP_502_BAD_GATEWAY: {"description": "Grok upstream failure"},
    },
    summary="Generate image via Grok",
    description=(
        "Generate one or more images using xAI's `grok-imagine-image-pro` model."
    ),
)
async def create_grok_image(
    request: GrokImageGenerationRequest,
) -> GrokImageGenerationResponse:
    """Return image URLs generated by Grok."""
    try:
        return await ai_service.generate_grok_image(request)
    except httpx.HTTPStatusError as exc:
        raise _map_grok_http_error(exc, "generate an image") from exc
    except httpx.RequestError as exc:
        raise _map_grok_request_error(exc, "generate an image") from exc


@router.post(
    "/grok-video-generation",
    response_model=GrokVideoGenerationResponse,
    responses={
        status.HTTP_200_OK: {"description": "Grok video generation completed or timed out"},
        status.HTTP_502_BAD_GATEWAY: {"description": "Grok upstream failure"},
    },
    summary="Generate video via Grok",
    description=(
        "Submit a generation request to xAI's `grok-imagine-video` model and poll until completion."
    ),
)
async def create_grok_video(
    request: GrokVideoGenerationRequest,
) -> GrokVideoGenerationResponse:
    """Return Grok video generation status and URL when available."""
    try:
        return await ai_service.generate_grok_video(request)
    except httpx.HTTPStatusError as exc:
